...
7. Success Criteria`,
    ...
    findSolidityFilesRecursive(dir, contracts) {
...
${analysis.modernization.recommendations.map((rec, i) => `${i + 1}. ${rec}`).join('\n')}
...
Recommendations:
${analysis.modernization.recommendations.filter((r) => r.includes('🛡️') || r.includes('🔐') || r.includes('⚠️')).join('\n')}
...
${analysis.dependencies.outdated.length > 0 ? analysis.dependencies.outdated.map((dep) => `- ${dep}`).join('\n') : '- None detected'}

Recommended Actions:
${!analysis.dependencies.foundry ? '- Install Foundry for better tooling\n' : ''}${!analysis.dependencies.openzeppelin ? '- Install OpenZeppelin for security patterns\n' : ''}${analysis.dependencies.outdated.map((dep) => `- Update ${dep}\n`).join('')}`;
    }
    async estimateGasSavings(contractAddress) {
