...
async function analyzeCommand(analyzer) {
    console.log('\n🔍 Starting analysis...\n');
    const analysis = await analyzer.analyzeDapp();
    analyzer.printReport(analysis);
    await prompt('\nPress Enter to continue...');
}
async function generateScriptCommand(analyzer) {
...
async function autoUpgradeCommand(analyzer) {
    console.log('\n🚀 AUTO-UPGRADE WIZARD\n');
    const analysis = await analyzer.analyzeDapp();
...
async function securityAuditCommand(analyzer) {
    console.log('\n🔒 SECURITY AUDIT\n');
    const analysis = await analyzer.analyzeDapp();
    ...
    if (!analysis.security.hasAccessControl)
        missingFeatures.push('Access Control');
    if (!analysis.security.hasPausable)
        missingFeatures.push('Pausable');
    if (!analysis.security.hasReentrancyGuard)
        missingFeatures.push('Reentrancy Guard');
...
async function codeStyleCommand(analyzer) {
...
async function dependencyCheckCommand(analyzer) {
    console.log('\n📦 DEPENDENCY CHECK\n');
    const analysis = await analyzer.analyzeDapp();
...
//# sourceMappingURL=modernizeCli.js.map